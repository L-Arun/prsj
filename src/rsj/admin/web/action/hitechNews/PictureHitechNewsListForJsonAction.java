...
public class PictureHitechNewsListForJsonAction extends BaseAction {
	private static final long serialVersionUID = 2436161530465382824L;
	private Logger logger = LoggerFactory.getLogger(PictureHitechNewsListForJsonAction.class);
	...
	public String handle() {
		...
		hitechNewses = hitechNewsService.pictureNewsListForJson(YesNoStatus.YES, YesNoStatus.YES, newsSize);
		if (hitechNewses != null && hitechNewses.size() > 0) {
			...
			for (HitechNews hitechNews : hitechNewses) {
				...
				j.put("imagePath", hitechNews.getImagePath());
				jsonArray.add(j);
			}
