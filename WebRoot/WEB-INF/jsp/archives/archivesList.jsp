...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
  		<div>
  			<div class="titlediv">您所在的位置：档案管理->收文管理->文件列表</div>
  			<div class="add_div_margin"><a href="${ctx}/archives/archives.do?action=input" class="easyui-linkbutton" iconCls="icon-add">添加文件</a></div>
  			<div>
  				<div>
  				...
  				<div>
  					...
  					<table cellpadding="0" cellspacing="0" border="0" class="querytab">
			    		...
			    		<s:iterator value="archiveses" status="index">
						<tr class="beover">
			    			<td>${index.count}</td>
			    			<td>${id}</td>
			    			<td>${number}</td>
			    			<td><a href="${ctx}/archives/archives.do?action=view&archives.id=${id}">${title}</a></td>
			    			<td><s:date name="createTime" format="yyyy-MM-dd" /></td>
			    			<td>${isDigital.name}</td>
			    			<td><a href="${ctx}/archives/archives.do?action=input&archives.id=${id}">修改</a></td>
			    		</tr>
			    		</s:iterator>
			    	</table>
			    	</div>
